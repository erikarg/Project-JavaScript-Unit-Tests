...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  const returnedObject = createMenu({
    food: { coxinha: 5.5, hamburguer: 6.2, kibe: 3.2 },
    drink: { suco: 4.5, refrigerante: 4.0, meiotinha: 1.5 },
  });
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(returnedObject.fetchMenu()).toEqual({
      food: { coxinha: 5.5, hamburguer: 6.2, kibe: 3.2 },
      drink: { suco: 4.5, refrigerante: 4.0, meiotinha: 1.5 },
    });
    ...
    returnedObject.order('refrigerante');
    expect(returnedObject.consumption).toEqual(['refrigerante']);
    // TESTE 6: Verifique se, ao adicionar três pedidos, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos.
    const firstOrder = createMenu({ food: {}, drink: {} });
    firstOrder.order('coxinha');
    firstOrder.order('refrigerante');
    firstOrder.order('hamburguer');
    expect(firstOrder.consumption).toStrictEqual([
      'coxinha',
      'refrigerante',
      'hamburguer',
    ]);
    // TESTE 7: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    const secondOrder = createMenu({ food: {}, drink: {} });
    secondOrder.order('coxinha');
    secondOrder.order('refrigerante');
    secondOrder.order('coxinha');
    expect(secondOrder.consumption).toStrictEqual([
      'coxinha',
      'refrigerante',
      'coxinha',
    ]);
    ...
    const thirdOrder = createMenu({
      food: { coxinha: 5.5, hamburguer: 6.2, kibe: 3.2 },
      drink: { suco: 4.5, refrigerante: 4.0, meiotinha: 1.5 },
    });
    thirdOrder.order('kibe');
    thirdOrder.order('meiotinha');
    expect(thirdOrder.pay()).toBe(4.7 + 4.7 * 0.1); // Consideração aos 10% de serviço
  });
});
