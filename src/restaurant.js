...
const consumedItems = (order, parameter) => {
  let sum = 0;
  for (let key in parameter) {
    if (Object.keys(parameter[key]).indexOf(order) > -1) {
      sum += parameter[key][order];
    }
  }
  return sum;
};
...
const createMenu = (parameter) => {
  ...
  const returnedObject = {
    fetchMenu: () => parameter,
    consumption: [],
    order: (consumed = (string) => returnedObject.consumption.push(string)),
    pay: (value = () => {
      let sum = 0;
      for (let i = 0; i < returnedObject.consumption.length; i += 1) {
        let order = returnedObject.consumption[i];
        sum += consumedItems(order, parameter);
      }
      return sum + 0.1 * sum;
    }),
  };
